...
> ISO 19650 準拠支援 BIM 情報管理プラットフォームの設計思想と構造
> （第三者による適合性評価・認証は未取得）
...
1. **二層準拠モデル**: ISO 19650 準拠支援を「システム実装」と「組織運用」に分離。システムは統制を支援し、契約・教育・例外承認は運用で担保（完全準拠の第三者認証は未取得）
...
Login → bcrypt 検証 → JWT (access + refresh) 発行
       → 以降 Bearer Token → get_current_user（revoked チェック）→ User 解決

refresh: ローテーション方式（提示された refresh token は失効化して新ペアを発行）
logout: アクセス・リフレッシュ両トークンの jti を失効化
rate limit: ログイン/登録は IP 単位のスライディングウィンドウ制限（単一プロセス）
```
