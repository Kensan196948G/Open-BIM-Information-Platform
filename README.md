...
- 承認依頼・承認結果をアプリ内通知で受信（未読バッジ・一覧・既読管理）

**情報セキュリティ**
- 公開 / 限定 / 機密 / 制限付き の4段階のアクセス区分
- 組織・プロジェクトごとのアクセス権限設定
- ロール（member / reviewer / org_admin）による操作権限の強制
  （承認・公開・保管は reviewer 以上、プロジェクト・命名規則の管理は org_admin 以上）
...
**検索・版管理**
- 識別子・タイトルによるコンテナ検索
- アップロードごとの版（P01.01 / P01.02…）を自動記録し、改訂履歴として表示
...
## MVP / Prototype（2026-08-14）

- 関係者レビュー用の **MVP / Prototype** 環境を整備しました
  （[MVP デモガイド](docs/MVP_DEMO.md)）。
